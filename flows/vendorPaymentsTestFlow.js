...
async function runVendorPaymentsTestFlow({ filter, top, skip }) {
    ...
    try {
        ...
        logStep("STEP 2", "Fetching Vendor Payments from SAP", {
            filter,
            top,
            skip
        });
        ...
        logStep("STEP 2 RESULT", "SAP Payments received", {
        ...
        for (const payment of mapped) {
            ...
            logStep(
                "MAPPED PAYMENT",
                `Payment ${payment.DocEntry} mapped`,
                {
        ...
        for (const payment of mapped) {
            ...
            if (errorMessage) {
                ...
                logStep("VALIDATION OK", "Payment passed validation", {
                    DocEntry: payment.DocEntry
                });
        ...
        for (const payment of paymentsWithError) {
            ...
            logStep("STEP 5 RESULT", "SAP payment updated", {
                DocEntry: payment.DocEntry
            });
        }
        ...
        logStep("STEP 7", "Logging out from SAP");
